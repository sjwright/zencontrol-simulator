...
- Group scene recall emits `SCENE_CHANGE`, `LEVEL_CHANGE_V2`, and `COLOUR_CHANGE`
  for the group and each member (level only when member destinations agree for
  the group wire; colour when members agree)
- `controller.dim_time_ms` applies a fade on scene recall; when greater than
  ...
  destination) until complete — matching real controllers
...
| `0x28` | QUERY_OPERATING_MODE_BY_ADDRESS        | No         |                                                           |
| `0x29` | OVERRIDE_DALI_BUTTON_LED_STATE         | No         |                                                           |
| `0x30` | QUERY_LAST_KNOWN_DALI_BUTTON_LED_STATE | No         |                                                           |
...
| `0xA0` | DALI_INHIBIT                           | Partial    | Timed inhibit flag; no sensor→load automation             |
...
| `0xB6` | QUERY_VIRTUAL_INSTANCES                | No         |                                                           |
